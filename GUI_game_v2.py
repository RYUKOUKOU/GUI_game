...
class MainWindow:
    ...
    def __addComponets(self):
        ...
        self.shuffle_button = tk.Button(self.root, text="Shuffle", command=self.use_shuffle_tool)
        self.shuffle_button.pack(side=tk.BOTTOM, pady=10)

    def use_shuffle_tool(self):
        if self.isGameStart:
            icons = [self.classmap[y][x] for y in range(self.gameSize) for x in range(self.gameSize)]
            random.shuffle(icons)
            index = 0
            for y in range(self.gameSize):
                for x in range(self.gameSize):
                    self.classmap[y][x] = icons[index]
                    index += 1
            self.drawMap()
